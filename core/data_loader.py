...
class DataLoader:
    ...
    def load_data(
        ...
        try:
            ...
            self.data = pd.read_csv(
                file_path, delimiter=delimiter, encoding=encoding, low_memory=low_memory
            )
            ...
            # Log success
            logger.info(
                f"Successfully loaded {len(self.data)} records with {len(self.data.columns)} columns"
            )
    ...
    def validate_schema(
        ...
        validation_result = {
            ...
            "match_percentage": (
                len(actual_columns & expected_columns_set) / len(expected_columns_set)
            )
            * 100,
        }
    ...
    def get_data_info(self) -> Dict[str, Union[Tuple, List, int, float]]:
        ...
        missing_percentage = (
            (missing_values / total_cells * 100) if total_cells > 0 else 0
        )
    ...
    def get_column_types(self) -> Dict[str, List[str]]:
        ...
        column_types = {"numerical": [], "categorical": [], "datetime": [], "other": []}
